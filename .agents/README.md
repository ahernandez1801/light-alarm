...
| --------------- | ---------------------------------------------------------------- | -------------------- |
| `<domain>`      | the integration domain, i.e. the `DOMAIN` constant in `const.py` | `ha_alarm_clock`     |
| `{ClassPrefix}` | the class name prefix used by every integration class            | `SunriseAlarm`       |
